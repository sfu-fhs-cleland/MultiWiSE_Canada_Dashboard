...
theme <- bslib::bs_add_rules(theme, "
  ...
  #upload_busy .spinner-border { width: 1rem; height: 1rem; }
")
...
")),
      ...
      tags$div(
        ...
        style = "position: fixed; right: 18px; bottom: 18px; z-index: 1080; display: none; padding: 10px 12px;",
        tags$div(
          style = "display:flex; align-items:center; gap:10px;",
          # spinner gets an id so we can hide it when ready
          tags$div(id = "upload_spinner", class = "spinner-border spinner-border-sm", role = "status", `aria-hidden` = "true"),
          tags$div(
            # title gets an id so we can swap the text
            tags$b(id = "upload_busy_title", "Preparing your file..."),
            tags$br(),
...
<p>If any of the above conditions are not met, the dashboard will either: (1) automatically handle the issue and store an associated warning message detailing how the issue was addressed, or (2) stop running entirely or skip the problematic individual(s) and store an error message detailing what the issue was.</p>"),
          tags$h5(HTML("<i>Running the dashboard for >500,000 individuals</i>")),
          HTML("
<p>If you would like to process more than 500,000 indviduals, you can either: 1) Split your Exposure Window and Residential History files into multiple files such that each file contains ≤500,000 unique <code>IndividualID</code>s, or 2) Run the dashboard locally on your machine. To run the dashboard locally, download the code and files provided here: <a href='https://github.com/sfu-fhs-cleland/MultiWiSE_Canada_Dashboard' target='_blank'>https://github.com/sfu-fhs-cleland/MultiWiSE_Canada_Dashboard</a>. You can then open app.R in RStudio, hit 'Run App,' and proceed with processing your files.</p>"),
...
server <- function(input, output, session) {
  ...
  # --- REPLACE JUST THESE THREE HELPERS ---
  
  # When a file chooser changes and we might be parsing, show spinner + "Preparing..."
  show_upload <- function(msg = NULL) {
    shinyjs::html("upload_busy_title", "Preparing your file...")
    shinyjs::html("upload_busy_text", msg %||% "Files are being processed, hang tight...")
    shinyjs::show("upload_spinner")
    shinyjs::show("upload_busy")
  }
  
  # When BOTH files are valid, flip to a finished state (no spinner, single message)
  set_upload_ready <- function() {
    shinyjs::html("upload_busy_title", "All done... Ready to calculate metrics")
    shinyjs::html("upload_busy_text", "")  # no secondary line once we're done
    shinyjs::hide("upload_spinner")
    shinyjs::show("upload_busy")
  }
  
  # Generic hide
  hide_upload <- function() {
    shinyjs::hide("upload_busy")
    # also reset spinner/title so a future upload starts clean
    shinyjs::html("upload_busy_title", "Preparing your file...")
  ...
  update_ready_state <- function() {
    ...
    if (v$file_ok) {
      ...
      set_upload_ready()            # show “Ready to calculate metrics!”
      v$files_ready_popup_shown <- TRUE
    } else {
      shinyjs::disable("run_analysis")
      output$status_line <- renderText("")
      hide_upload()                 # hide card until both present/processing
  ...
  #   msg_inline <- sprintf(
  #     "Maximum number of IndividualIDs is %s. Your files contain %s unique IndividualIDs. Please see 'About' for how to run the dashboard for >%s individuals.",
  #     format(limit, big.mark=","), format(n, big.mark=","), format(limit, big.mark=",")
  #   )
  ...
  observeEvent(input$res_file, ignoreInit = TRUE, {
    ...
    invisible(tryCatch({
      ...
      #     block_id_cap(n_ids); return(invisible())
      #   } else {
      #     v$too_many_ids <- FALSE
      #   }
      # }
  ...
  observeEvent(input$run_analysis, {
    tryCatch({
      ...
      shinyjs::addClass("results_hdr",  "muted")
      update_warn_badge(0)
      output$status_line <- renderText("Checking data...")
      
      run_dir <- file.path(out_dir_default, paste0("run_", format(Sys.time(), "%Y%m%d_%H%M%S")))
      dir.create(run_dir, recursive = TRUE, showWarnings = FALSE)
      
      withProgress(message = "Checking data...", value = 0, {
        
        setProgress(value = 0, message = "Checking data...", detail = "")
        ...
        # ids_union_n <- length(setdiff(union(res_ids_all, exp_ids_all), NA_character_))
        # if (ids_union_n > MAX_UNIQUE_INDIVIDUALS) {
        #   block_id_cap(ids_union_n)
        #   return(invisible())
        # }
        # ----------------------------------------------------------------------------- 
        
        setProgress(value = 0, message = "Setting up data for processing...", detail = "")
        ...
        for (id in ids_all) {
          i <- i + 1L
          setProgress(value = (i-1)/max(1,n), message = "Processing data...",
                      detail = sprintf("Individual %d of %d...", i, n))
